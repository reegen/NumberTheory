 ...
 *
 **************
 * The algorithm is - 1. l <- log n 2. for i<-2 to l a. if an is a power fo l
 * return COMPOSITE 3. r <- 2 4. while r < n a. if gcd( r, n) != 1 return
 * COMPSITE b. if sieve marked n as PRIME q <- largest factor (r-1) o < - r-1 /
 * q k <- 4*sqrt(r) * l if q > k and n <= r return PRIME c. x = 2 d. for a <- 1
 * to k if (x + a) ^n != x^n + mod (x^r - 1, n) return COMPOSITE e. return PRIME
 */
...
public class AKS {
	...
	/*
	 * creates a sieve array that maintains a table for COMPOSITE-ness or
	 * possibly PRIME state for all values less than SIEVE_ERATOS_SIZE
	 */
	...
	/*
	 * array to populate sieve array the sieve array looks like this
	 * 
	 * y index -> 0 1 2 3 4 5 6 ... n x index 1 | 2 T - T - T ... \/ 3 T - - T
	 * ... 4 T - - ... . T - ... . T ... n
	 * 
	 *
	 *
