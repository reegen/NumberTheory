...
public class Poly {
	...
	/**
	 * Create a zero polynomial with a coefficient array of a given size
	 * 
	 * @param size
	 */
	...
	/**
	 * Obtain coefficient for a given term of the polynomial
	 * 
	 * @param d
	...
	public String toString() {
		...
		for (int i = this.degree; i >= 1; i--)
			if (monos[i].compareTo(BigInteger.ZERO) != 0)
				s += (monos[i].compareTo(BigInteger.ONE) == 0 ? "" : monos[i]) + "x^" + i + " + ";
	...
	/**
	 * Synchronize the degree of the polynomial with the coefficient array
	 * length and coefficient values
	 * 
	 * @param p
	 */
	...
	/***
	 * Exponentiate a polynomial with the coefficients mod a constant and the
	 * polynomial mod another polynomial
	 * 
	 * @param exponent
	 * @param mPoly
	 * @param mBigInteger
